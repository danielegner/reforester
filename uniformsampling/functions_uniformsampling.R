...
# functions_uniformsampling.R

# This script is almost the same as "functions.R", with some modifications
# to make it use only a single uniform sampling rate instead of the six
# empirically-derived sampling models. It is called by "main_uniformsampling.R".
...
GetIntervalsSamplingRates <- function(importfilename, whichcol) {
	...
	# Fill third column with a single, constant sampling rate of 0.5
	intervalsinfo[, 3] <- 0.5
...
CalculateSlopeDifference <- function(kickedtree, etimes, origrectree, flstimes, sampmodel) {
	...
	outputs <- list(slopedifference.abs, slopedifference.rel)
	return(outputs)
...
RunSimulations <- function(divmodel, allsamplemodels) {
	...
	outputs <- list(allsamplemodels.slopediffs.abs, allsamplemodels.slopediffs.rel)
	return(outputs)
