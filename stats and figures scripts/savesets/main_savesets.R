...
# dissertation. It calls "functions_savesets.R".
...
source("functions_savesets.R")   # For all custom functions
...
if (divmodelchoice == 1) {
	...
	while (successfulruns.divdep < numberofsimruns) {
		if (is.null(attr(try(outputs <- RunSimulations(divmodel = "diversitydependent", allsamplemodels = samplingmodels)), "class"))) {
			...
			trutree <- outputs[[3]]
			tdl_allsix <- outputs[[4]]
			ltt_allsix <- outputs[[5]]
			trr_allsix <- outputs[[6]]
